"""
kCTF / Kubernetes Adapter
==========================
kCTF is Google's CTF infrastructure framework. It uses Kubernetes with nsjail
(namespace-based jail) inside each pod for challenge isolation.
...
- Pods are placed in the kctf namespace with restrictive NetworkPolicy
- No pod-to-pod traffic (default deny east-west)
- No access to Kubernetes API from inside the pod
- Resource limits enforced by Kubernetes LimitRange

See infra/kctf/README.md for the full fresh cluster setup.
"""
import asyncio
import structlog
...
class KCTFAdapter(RuntimeAdapter):
    def __init__(self):
    ...
    async def launch(self, req: LaunchRequest) -> LaunchResult:
        ...
        await asyncio.to_thread(self._create_kctf_pod, req, name, host_port)
        ...
        log.info("kctf instance launched", instance_id=req.instance_id, pod=name)
    ...
    def _create_kctf_pod(self, req: LaunchRequest, name: str, host_port: int):
